...
export default function MySubscriptionsPage() {
  ...
  const handleAddressChange = () => {
      ...
      toast("📍 Lütfen Profil sayfasından adresinizi güncelleyin.", { icon: 'ℹ️' });
      // router.push('/profile');
  };
    ...
    <div className="min-h-screen bg-[#f8f9fa] font-sans">
      ...
      <div className="bg-gradient-to-r from-green-800 to-green-600 text-white pt-24 pb-32 px-6 shadow-xl relative overflow-hidden">
         ...
         <div className="container mx-auto max-w-4xl relative z-10 flex justify-between items-end">
             <div>
                <h1 className="text-4xl font-black tracking-tight mb-2">Aboneliklerim</h1>
                <p className="text-green-100 text-lg opacity-90">Aktif paketlerini yönet, teslimat tarihlerini güncelle.</p>
             </div>
             <button onClick={() => router.push('/')} className="bg-white/10 hover:bg-white/20 text-white text-sm font-bold px-4 py-2 rounded-lg transition backdrop-blur-sm border border-white/20">
                ← Ana Sayfaya Dön
             </button>
         </div>
      </div>
      ...
      <div className="container mx-auto max-w-4xl px-6 -mt-20 pb-20 relative z-20 space-y-6">
        ...
        {subs.length > 0 ? (
            subs.map((sub) => {
                const isActive = sub.status === 'ACTIVE' || sub.status === 'PENDING'; // Aktif mi?

                return (
                    <div key={sub.id} className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden transition hover:shadow-2xl">
                        {/* Kart Başlığı (Durum Barı) */}
                        <div className={`h-2 w-full ${isActive ? 'bg-green-500' : 'bg-red-500'}`}></div>
                        ...
                        <div className="p-8 flex flex-col md:flex-row gap-8">
                            ...
                            <div className="flex-1 flex items-start gap-6">
                                ...
                                <div>
                                    <div className="flex items-center gap-3 mb-1">
                                        <h3 className="text-2xl font-black text-gray-800">{sub.product?.name || "Sürpriz Kutu"}</h3>
                                    ...
                                    <p className="text-gray-400 font-bold text-sm tracking-wide mb-4">#{sub.id.toString().padStart(6, '0')} • Aylık Plan</p>
                                    <div className="text-2xl font-black text-gray-900">
                                        {sub.product?.price} ₺ <span className="text-sm text-gray-400 font-medium">/ay</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 border-l border-gray-100 pl-0 md:pl-8 space-y-6">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        <p className="text-xs text-gray-400 font-bold uppercase tracking-wider">Sonraki Yenilenme</p>
                                        <p className="font-bold text-gray-800 text-lg">
                                            {sub.nextPaymentDate 
                                                ? new Date(sub.nextPaymentDate).toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', year: 'numeric' })
                                                : '-'}
                                        </p>
                                        {isActive && <p className="text-xs text-green-600 font-bold mt-1">Otomatik yenilenecek</p>}
                                    </div>
                                </div>
                                ...
                                <div className="flex items-start gap-4">
                                    <div className="bg-gray-50 p-2 rounded-lg text-xl">📍</div>
                                    <div>
                                        <p className="text-xs text-gray-400 font-bold uppercase tracking-wider">Teslimat Adresi</p>
                            ...
                            <div className="w-full md:w-48 flex flex-col gap-3 justify-center">
                                {isActive ? (
                                    ...
                                    <div className="text-center py-4 bg-gray-50 rounded-xl border border-gray-100">
                                        <p className="text-gray-400 font-bold text-sm">Bu üyelik pasif.</p>
                                        <button onClick={() => router.push('/')} className="text-green-600 text-xs font-bold underline mt-2 hover:text-green-700">
                                            Tekrar Abone Ol
                                        </button>
                                    </div>
                                )}
            ...
            <div className="bg-white rounded-3xl shadow-xl p-12 text-center border border-gray-100 relative overflow-hidden group cursor-pointer" onClick={() => router.push('/#paketler')}>
                ...
                <h3 className="text-2xl font-black text-gray-800 mb-2">Henüz Aktif Aboneliğin Yok</h3>
                <p className="text-gray-500 max-w-md mx-auto mb-8">
                    Minik dostunu sevindirmek için hemen bir paket seç! İlk kutuda sürpriz hediyeler seni bekliyor.
                </p>
