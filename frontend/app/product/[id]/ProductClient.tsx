    ...
    return (
      <div className="bg-orange-50 border border-orange-100 p-8 rounded-xl text-center animate-fade-in mt-4">
        ...
        <p className="text-gray-600 text-sm leading-relaxed max-w-md mx-auto">
          Taksit seçenekleri banka anlaşmaları gereği sadece{" "}
          <strong>3, 6, 9 veya 12 aylık Peşin Ödeme (Tek Çekim)</strong>{" "}
          planlarında geçerlidir.
          <br />
          <br />
          Taksitleri görmek için lütfen yukarıdan uzun dönemli bir paket
          seçiniz.
        </p>
      </div>
    );
  ...
  return (
    <div className="overflow-x-auto border border-gray-100 rounded-xl mt-4 animate-fade-in">
      <div className="bg-blue-50 p-4 text-sm text-blue-800 mb-4 rounded-lg flex items-center gap-2 m-4">
        <span>💡</span>
        <strong>Bilgi:</strong> Aşağıdaki taksit seçenekleri, ödemenin tamamını
        peşin yaptığınızda geçerlidir.
      </div>
...
function ProductDetailContent() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (token) {
          ...
          const petsRes = await fetch(`${API_URL}/users/pets`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (petsRes.ok) {
  ...
  useEffect(() => {
    if (duration === 1) setPaymentType("upfront");
  }, [duration]);
  ...
  useEffect(() => {
    if (dateParts.day && dateParts.month && dateParts.year) {
      ...
      const formattedDay =
        Number(dateParts.day) < 10 ? `0${dateParts.day}` : dateParts.day;
      setPetData((prev) => ({
        ...prev,
  ...
  const calculatePrice = (months: number) => {
    if (!product)
      return { total: 0, monthly: 0, discountRate: 0, originalTotal: 0 };
  ...
  const getOtherIcon = () =>
    petData.otherType && OTHER_ICONS[petData.otherType]
      ? OTHER_ICONS[petData.otherType]
      : "🦜";

  const handleAddAllergy = () => {
    if (
      petData.allergyInput.trim() &&
      !petData.allergies.includes(petData.allergyInput.trim())
    ) {
      setPetData({
        ...petData,
        allergies: [...petData.allergies, petData.allergyInput.trim()],
        allergyInput: "",
      });
    }
  };
  const removeAllergy = (a: string) => {
    setPetData({
      ...petData,
      allergies: petData.allergies.filter((x) => x !== a),
    });
  };
  ...
  const handleNextStep = async () => {
    if (step === 1) {
      if (!localStorage.getItem("token") && duration > 1) {
      ...
      if (isNewPetMode || savedPets.length === 0) {
        if (!petData.name || !petData.breed || !petData.weight) {
          toast.error("Bilgileri doldurmalısın.");
          return;
        }
      }
      if (localStorage.getItem("token") && isNewPetMode) {
        if (!(await saveNewPet())) return;
      }
      setStep(3);
    } else {
      if (!product) return;
      ...
      const finalPrice =
        paymentType === "monthly" ? Number(product.price) : priceInfo.total;
  ...
  // STYLES
  const inputStyle =
    "w-full p-4 bg-white border border-gray-200 rounded-xl font-bold text-gray-900 outline-none focus:border-green-500 transition shadow-sm placeholder:text-gray-400";
  const selectStyle =
    "w-full p-3 bg-white border border-gray-200 rounded-xl font-bold text-gray-900 outline-none focus:border-green-500 transition cursor-pointer shadow-sm";
  ...
  return (
    <main className="min-h-screen bg-[#f8f9fa] font-sans pb-10">
      ...
      <UpsellModal
        ...
        savingsAmount={currentPriceInfo.originalTotal - currentPriceInfo.total}
      />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <button
          onClick={() => router.back()}
          className="mb-6 text-gray-500 font-bold hover:text-green-600 transition"
        >
          ← Geri
        </button>
          ...
          <div className="lg:col-span-8">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 mb-6 animate-fade-in flex items-start gap-4 shadow-sm">
                ...
                <div>
                  <h4 className="font-bold text-blue-900 text-lg">
                    Paket Yükseltme Modu
                  </h4>
                  <p className="text-blue-700 text-sm mt-1 leading-relaxed">
                    Şu an <strong>{petData.name || "Dostunuz"}</strong> için
                    paket yükseltme işlemi yapıyorsunuz.
                    <br />
                    Hesaplanan İade Tutarı:{" "}
                    <span className="font-black bg-blue-100 px-2 py-0.5 rounded">
                      ₺{calculatedRefund.toFixed(2)}
                    </span>
                    <br />
                    <span className="text-xs opacity-75">
                      Bu tutar ödeme ekranında toplam fiyattan düşülecektir.
                    </span>
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden min-h-[600px] flex flex-col">
              ...
              <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center text-sm">
                <span
                  className={
                    step >= 1 ? "text-green-600 font-bold" : "text-gray-400"
                  }
                >
                  1. Süre
                </span>
                <span
                  className={
                    step >= 2 ? "text-green-600 font-bold" : "text-gray-400"
                  }
                >
                  2. Dostun
                </span>
                <span
                  className={
                    step >= 3 ? "text-green-600 font-bold" : "text-gray-400"
                  }
                >
                  3. Özet
                </span>
                  ...
                  <div className="animate-fade-in space-y-6">
                      ...
                      {[1, 3, 6, 9, 12].map((m) => {
                        ...
                        return (
                          <button
                            ...
                            className={`p-4 rounded-2xl border-2 transition relative h-40 flex flex-col justify-between ${
                              duration === m
                          ...
                          >
                            {info.discountRate > 0 && (
                              <span className="absolute -top-2 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-[10px] px-2 rounded-full">
                                %{info.discountRate} İNDİRİM
                              </span>
                            )}
                    ...
                    <div className="text-right">
                      <button
                        ...
                        className="bg-gray-900 text-white px-8 py-3 rounded-xl font-bold"
                      >
                        Devam Et 👉
                      </button>
                    </div>
                  ...
                  <div className="animate-fade-in space-y-6">
                        ...
                        {savedPets.map((pet) => (
                          <div
                            ...
                            onClick={() => {
                              ...
                              if (upgradeMode && pet.id !== preSelectedPetId)
                                return;
                    ...
                    {!upgradeMode && (
                      <button
                        ...
                        className={`w-full p-3 rounded-xl border-2 border-dashed border-gray-300 text-gray-500 font-bold mb-6 ${
                          isNewPetMode
                    ...
                    {(isNewPetMode || savedPets.length === 0) && (
                      <div className="space-y-4">
                        <div className="flex gap-3">
                          ...
                          <div className="relative flex-1">
                            <button
                              ...
                              className={`w-full h-full py-3 rounded-xl font-bold border-2 flex items-center justify-center gap-1 ${
                                petData.type !== "kopek" &&
                                petData.type !== "kedi"
                                  ? "border-green-500 bg-green-50 text-green-800"
                                  : "border-gray-200"
                              ...
                              <div className="absolute top-full left-0 w-full mt-2 bg-white border border-gray-100 shadow-xl rounded-xl z-20 overflow-hidden">
                                {Object.keys(OTHER_ICONS).map((t) => (
                                  <button
                                    ...
                                    className="w-full text-left px-4 py-3 hover:bg-green-50 font-bold text-gray-700 border-b border-gray-50 flex gap-2"
                                  >
                        ...
                        <div className="flex gap-4">
                          <label
                            className={`flex-1 p-3 rounded-xl border-2 cursor-pointer flex items-center justify-center gap-2 transition shadow-sm ${
                              petData.isNeutered
                        ...
                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                          ...
                          <div className="flex gap-2 mb-2">
                            <input
                              ...
                              className="flex-grow p-3 bg-white rounded-xl outline-none border border-gray-300 font-bold text-gray-900"
                              placeholder="Örn: Tavuk..."
                              onKeyDown={(e) =>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {petData.allergies.map((a, i) => (
                              <span
                                key={i}
                                className="bg-white border px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-2 text-gray-700"
                              >
                                🚫 {a}{" "}
                                <button
                  ...
                  <div className="animate-fade-in text-center space-y-6">
                    ...
                    {duration > 1 && (
                      <div className="bg-blue-50 p-4 rounded-2xl mb-4 border border-blue-100">
                        <p className="text-left text-sm font-bold text-blue-900 mb-3 ml-1">
                          Ödeme Tercihi Seçiniz:
                        </p>
                        <div className="flex gap-3">
                          <button
                        ...
                        {!isLoggedIn && (
                          <p className="text-xs text-red-500 mt-2 text-left ml-1">
                            * Aylık ödeme için giriş yapmalısınız.
                          </p>
                        )}
                      </div>
                    )}
                      ...
                      <div className="flex justify-between items-end pt-2">
                        <div>
                          ...
                          <span className="text-3xl font-black text-gray-900 tracking-tight">
                            ...
                            {paymentType === "monthly"
                              ? Number(product.price).toFixed(2)
                              : currentPriceInfo.total.toFixed(2)}
                          </span>
                        </div>
                        <div className="text-right">
                          {paymentType === "monthly" ? (
                      ...
                      {upgradeMode && (
                        <div className="mt-4 pt-3 border-t border-gray-200">
                          ...
                          <p className="text-xs text-gray-400 mt-1 text-right">
                            Bu tutar ödeme sayfasında düşülecektir.
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <button
                      onClick={handleNextStep}
                      className="bg-gray-900 text-white w-full py-4 rounded-xl font-bold shadow-xl hover:bg-black transition transform active:scale-95 flex items-center justify-center gap-2"
                    >
                      <span>
                        {upgradeMode ? "Yükselt ve Öde" : "Ödemeye Geç"}
                      </span>
                      <span>➔</span>
                    </button>
              ...
              <div className="flex border-b border-gray-100">
                <button
                  ...
                  className={`flex-1 py-4 text-sm font-bold transition-all ${
                    activeTab === "desc"
                      ? "border-b-4 border-green-500 text-green-600 bg-green-50/50"
                    ...
                    {product.features && product.features.length > 0 && (
                      <div className="mt-4">
                        <h4 className="font-bold text-gray-900 mb-3">
                          Bu Kutuda Sizi Neler Bekliyor?
                        </h4>
                        <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {product.features.map((feature, idx) => (
                            <li
                              key={idx}
                              className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg border border-gray-100 text-gray-700 text-sm font-medium"
                            >
...
// ⚠️ BU BÖLÜME DİKKAT: Ana bileşeni dışarı aktarıyoruz
