...
// 👇 1. IMPORT EKLENDİ: Sepet Context'i
import { useCart } from "@/context/CartContext";

// Navbar için gerekli bileşenler
...
export default function ProductDetail() {
  ...
  // 👇 2. HOOK EKLENDİ: Sepete ekleme fonksiyonunu alıyoruz
  ...
  // 👇 İLERLEME FONKSİYONU (GÜNCELLENEN KISIM BURASI)
  const handleNextStep = async () => {
      ...
      // ADIM 3 -> CHECKOUT (GÜNCELLENDİ)
      else {
          // Güvenlik: Ürün yoksa işlem yapma
          if (!product) return;
          ...
          const cartItem = {
              ...
              // Fiyat hesaplaması: Aylıksa birim fiyat, Peşinse toplam fiyat
              price: paymentType === 'monthly' ? Number(product.price) : Number(product.price) * duration,
              image: product.image,
              ...
              // Pet detayları
              petId: selectedPetId, // Step 2'de set edilmiş olmalı
              petName: petData.name, // Görüntüleme için
                ...
                <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden min-h-[600px] flex flex-col">
                    ...
                    <div className="p-8 md:p-12 flex-grow flex flex-col justify-center">
                        ...
                        {step === 2 && (
                            <div className="animate-fade-in space-y-8">
                                <div>
                                    <h2 className="text-3xl font-black text-gray-900 mb-2">Bu Kutu Kimin İçin? 🐾</h2>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {savedPets.map(pet => (
                                            <div 
                                                ...
                                                className={`p-4 rounded-xl border-2 cursor-pointer transition-all flex items-center gap-4 ${selectedPetId === pet.id && !isNewPetMode ? 'border-green-500 bg-green-50 shadow-md' : 'border-gray-200 hover:border-green-200'}`}
                                            >
                                                <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-2xl shadow-sm">
                                                    {pet.type === 'kopek' ? '🐶' : pet.type === 'kedi' ? '🐱' : '🦜'}
                                                </div>
                                                <div>
                                                    <h4 className="font-bold text-gray-900">{pet.name}</h4>
                                                    <p className="text-xs text-gray-500">{pet.breed || pet.type} • {pet.weight}kg</p>
                                                </div>
                                    ...
                                    <div className="animate-slide-down border-t border-gray-100 pt-6">
                                            ...
                                            <div className="relative flex-1">
                                                ...
                                                {isOtherOpen && (
                                                    <div className="absolute top-full left-0 w-full mt-2 bg-white border border-gray-100 shadow-xl rounded-xl z-20 overflow-hidden">
                                                        {['Kuş', 'Hamster', 'Tavşan', 'Balık'].map((t) => (
                                                            <button key={t} onClick={() => {setPetData({...petData, type: 'diger', otherType: t}); setIsOtherOpen(false);}} className="w-full text-left px-4 py-3 hover:bg-green-50 hover:text-green-700 font-medium text-gray-600 transition border-b border-gray-50 last:border-0 flex items-center gap-2"><span>{OTHER_ICONS[t]}</span> {t}</button>
                                                        ))}
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <div className="mb-6">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                                {["1-5", "6-10", "11-15", "16-20"].map((range) => (
                                                    <button key={range} onClick={() => setPetData({...petData, shippingDate: range})} className={`py-2 px-1 rounded-lg font-bold border-2 transition text-xs ${petData.shippingDate === range ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-200 text-gray-500 hover:border-green-200'}`}>Her Ayın {range}'i</button>
                                                ))}
                                            </div>
                                        </div>
                                        <div className="space-y-4">
                                            <label className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition group bg-white">
                                                ...
                                                <span className="font-bold text-gray-700 group-hover:text-gray-900 text-sm">Dostum Kısırlaştırılmış</span>
                                            </label>
                                            <div className="p-4 border border-gray-200 rounded-xl bg-white">
                                                <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Alerjiler (Varsa)</label>
                                                <div className="flex gap-2">
                                                    <input type="text" value={petData.allergyInput} onChange={e => setPetData({...petData, allergyInput: e.target.value})} onKeyDown={e => e.key === 'Enter' && handleAddAllergy()} placeholder="Örn: Tavuk..." className="flex-1 p-2 bg-gray-50 border border-gray-200 rounded-lg font-medium outline-none focus:border-green-500 text-gray-900 text-sm" />
                                                    <button onClick={handleAddAllergy} className="px-4 bg-gray-200 text-gray-600 font-bold rounded-lg hover:bg-gray-300">+</button>
                                                </div>
                                                <div className="flex flex-wrap gap-2 mt-3">
                                                    {petData.allergies.map((allergy, idx) => (
                                                        <span key={idx} className="bg-red-50 text-red-600 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2 border border-red-100">{allergy}<button onClick={() => removeAllergy(allergy)} className="hover:text-red-800">×</button></span>
                                                    ))}
                                                </div>
                                            </div>
                                        </div>
        ...
        <div className="mt-20 border-t border-gray-200 pt-12">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 text-gray-600 leading-relaxed space-y-4">
                ...
                <ul className="list-disc pl-5 space-y-1">
                    <li>Her ay farklı ve heyecan verici konseptler.</li>
                    <li>Alerji durumuna göre özelleştirilebilir içerik.</li>
                    <li>Piyasa değerinin çok altında avantajlı fiyat.</li>
                    <li>Memnun kalmazsanız kolay iade ve iptal imkanı.</li>
                </ul>
            </div>
        </div>
      </div>
