...
export default function EditPetModal({ isOpen, onClose, onSuccess, petData }: EditPetModalProps) {
  ...
  const [formData, setFormData] = useState({
    name: "", type: "kopek", breed: "", weight: "", birthDate: "", isNeutered: false, allergies: "",
  });

  useEffect(() => {
    if (petData) {
      setFormData({
        name: petData.name || "", type: petData.type || "kopek", breed: petData.breed || "", weight: petData.weight || "",
        birthDate: petData.birthDate ? new Date(petData.birthDate).toISOString().split('T')[0] : "",
        isNeutered: petData.isNeutered || false, allergies: petData.allergies ? petData.allergies.join(", ") : "",
      });
    }
  }, [petData, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { toast.success("Bilgiler güncellendi! 🐾"); onSuccess(); onClose(); } 
      else { toast.error("Güncelleme başarısız."); }
    } catch (error) { toast.error("Bir hata oluştu."); }
  };

  if (!isOpen) return null;
  const inputClass = "w-full p-3 rounded-xl border-2 border-gray-100 bg-gray-50 text-gray-900 font-bold outline-none focus:border-green-500 focus:bg-white transition text-sm md:text-base";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-[2rem] w-full max-w-lg p-6 md:p-8 relative shadow-2xl max-h-[90vh] overflow-y-auto">
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 text-2xl bg-gray-50 w-8 h-8 rounded-full flex items-center justify-center">✕</button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div><label className="text-xs font-bold text-gray-400 mb-1 block">İSİM</label><input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className={inputClass} /></div>
                <div><label className="text-xs font-bold text-gray-400 mb-1 block">IRK</label><input type="text" value={formData.breed} onChange={e => setFormData({...formData, breed: e.target.value})} className={inputClass} /></div>
            </div>

            <div className="grid grid-cols-2 gap-4">
                <div><label className="text-xs font-bold text-gray-400 mb-1 block">KİLO (KG)</label><input type="number" step="0.1" value={formData.weight} onChange={e => setFormData({...formData, weight: e.target.value})} className={inputClass} /></div>
