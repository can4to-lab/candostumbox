...
export default function AddPetModal({
  ...
  const inputClass =
    "w-full p-3 md:p-4 rounded-xl border-2 border-gray-100 bg-gray-50 text-gray-900 font-bold outline-none focus:border-green-500 focus:bg-white transition placeholder:text-gray-400 text-sm md:text-base";
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto">
      ...
      <div className="bg-white rounded-[2rem] w-full max-w-lg relative shadow-2xl p-6 md:p-8 animate-fade-in-up my-auto">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 text-2xl w-8 h-8 flex items-center justify-center bg-gray-100 rounded-full transition"
        >
          &times;
        </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-green-600 text-white py-4 rounded-xl font-bold hover:bg-green-700 transition mt-4 shadow-lg shadow-green-200 text-lg"
          >
