    ...
    const fetchData = async () => {
      ...
      try {
        const [ordersRes, usersRes, productsRes] = await Promise.all([
          fetch("https://candostumbox-api.onrender.com/orders", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch("https://candostumbox-api.onrender.com/users", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ...
        const revenue = Array.isArray(orders)
          ? orders.reduce(
              (acc: number, o: any) => acc + Number(o.totalPrice),
              0,
            )
          : 0;
        const stockAlert = Array.isArray(products)
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              className="p-4 bg-gray-100 text-gray-800 rounded-xl hover:bg-gray-900 hover:text-white transition font-bold text-left flex items-center gap-2"
            >
              <span className="text-xl">📦</span> Bekleyen Siparişler
            </button>
            <button
              onClick={() => router.push("/admin/products")}
              className="p-4 bg-purple-100 text-purple-800 rounded-xl hover:bg-purple-600 hover:text-white transition font-bold text-left flex items-center gap-2"
            >
            ...
            >
              <span className="text-xl">🏷️</span> Otomatik İndirim
            </button>
            <button
              onClick={() => router.push("/admin/promo-codes")}
              className="p-4 bg-pink-100 text-pink-800 rounded-xl hover:bg-pink-600 hover:text-white transition font-bold text-left flex items-center gap-2"
            >
              <span className="text-xl">🎟️</span> Kampanya Kodları
            </button>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-gray-900 to-black p-8 rounded-3xl shadow-xl text-white relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-gray-400 text-sm mb-6">
              Backend ve Veritabanı bağlantıları aktif.
            </p>
