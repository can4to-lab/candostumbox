...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const urunleriGetir = async () => {
      try {
        const cevap = await fetch("http://localhost:3000/products");
      ...
      <nav className="bg-white shadow-sm sticky top-0 z-40 h-20">
        <div className="container mx-auto px-6 h-full relative flex items-center justify-between">
            ...
            <div className="flex items-center">
                ...
                <span className="ml-3 text-sm font-bold text-gray-600 hidden md:block cursor-pointer" onClick={() => setIsSideMenuOpen(true)}>MENÜ</span>
            </div>

            {/* 2. ORTA: LOGO (Tam Ortada) */}
            <div className="absolute left-1/2 transform -translate-x-1/2 flex items-center cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                <img src="/logo_arka.png" alt="CanDostumBox" className="h-10 md:h-12 w-auto" />
               <span className="ml-2 text-2xl font-black text-gray-800 tracking-tighter hidden lg:block">CanDostum<span className="text-green-600">Box</span></span>
            </div>
            ...
            <div className="flex items-center gap-6">
                ...
                >
                    ...
                    {isLoggedIn && <span className="absolute bottom-0 right-0 block h-2.5 w-2.5 rounded-full bg-green-500 ring-2 ring-white"></span>}
                </button>
      ...
      >
          <div className="flex flex-col h-full p-8">
              ...
              <div className="flex justify-between items-center mb-10">
                  ...
                  <button onClick={() => setIsSideMenuOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition">
              ...
              <nav className="flex flex-col space-y-6">
                  <button onClick={() => handleNavigation('/')} className="text-left text-2xl font-bold text-gray-900 hover:text-green-600 transition">Ana Sayfa</button>
                  <button onClick={() => handleNavigation('/paketler')} className="text-left text-2xl font-bold text-gray-900 hover:text-green-600 transition">Paketler</button>
                  <button onClick={() => handleNavigation('/neden-biz')} className="text-left text-2xl font-bold text-gray-900 hover:text-green-600 transition">Neden Biz?</button>
                  <button onClick={() => scrollToSection('sss')} className="text-left text-2xl font-bold text-gray-900 hover:text-green-600 transition">S.S.S.</button>
              </nav>
              ...
              <div className="mt-auto pt-8 border-t border-gray-100">
                  {isLoggedIn ? (
                      <div className="space-y-4">
                          <button onClick={() => handleNavigation('/profile')} className="block w-full text-left text-lg font-medium text-gray-700 hover:text-green-600">Hesabım</button>
                          <button onClick={handleLogout} className="block w-full text-left text-lg font-medium text-red-600 hover:text-red-700">Çıkış Yap</button>
                      </div>
                  ) : (
                      <div className="grid grid-cols-2 gap-4">
                          <button onClick={() => {setLoginOpen(true); setIsSideMenuOpen(false);}} className="bg-gray-100 text-gray-900 py-3 rounded-lg font-bold hover:bg-gray-200 transition">Giriş Yap</button>
                          <button onClick={() => {setRegisterOpen(true); setIsSideMenuOpen(false);}} className="bg-green-600 text-white py-3 rounded-lg font-bold hover:bg-green-700 transition">Kayıt Ol</button>
                      </div>
                  )}
              </div>
          </div>
 ...
 {/* --- ABONELİK SİSTEMİ BİLGİLENDİRME BANNERI (YENİ) --- */}
      ...
      <section id="paketler" className="container mx-auto px-4 py-24 bg-white">
        <div className="text-center mb-20 space-y-4">
           <h2 className="text-4xl md:text-5xl font-black text-gray-900">Size Uygun Paketi Seçin</h2>
           <p className="text-xl text-gray-500 font-medium max-w-2xl mx-auto">Dostunuzun boyutuna ve ihtiyacına en uygun kutuyu seçin, mutluluğu başlatın.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {visibleProducts.map((urun, index) => {
              ...
              return (
                <div key={urun.id} className={`relative bg-white rounded-[2.5rem] p-8 transition-all duration-500 flex flex-col
                    ${index === 1 
                ...
                `}>
                  ...
                  {stokYok && <div className="absolute top-6 right-6 bg-red-50 text-red-600 text-xs font-bold px-3 py-1 rounded-full border border-red-100">TÜKENDİ</div>}
                  {stokAz && <div className="absolute top-6 right-6 bg-orange-50 text-orange-600 text-xs font-bold px-3 py-1 rounded-full border border-orange-100 animate-pulse">Son {urun.stock} Adet</div>}

                  <div className="text-center mb-8 pt-4">
                      <h3 className="text-2xl font-black text-gray-900">{urun.name}</h3>
                      <p className="text-gray-500 text-sm mt-3 leading-relaxed min-h-[40px]">{urun.description}</p>
                  </div>
                  ...
                  <div className="flex justify-center items-end gap-1 mb-10 pb-10 border-b border-dashed border-gray-200">
                    ...
                    <span className="text-7xl font-black text-gray-900 tracking-tighter">{urun.price}</span>
                  ...
                  <ul className="space-y-4 mb-10 text-gray-600 text-left px-2 flex-grow">
                      <li className="flex items-center gap-3"><span className="flex-shrink-0 w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center text-xs font-bold">✓</span><span className="font-medium">Orijinal Lisanslı Ürünler</span></li>
                      <li className="flex items-center gap-3"><span className="flex-shrink-0 w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center text-xs font-bold">✓</span><span className="font-medium">Veteriner Kontrolünde</span></li>
                      <li className="flex items-center gap-3"><span className="flex-shrink-0 w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center text-xs font-bold">✓</span><span className="font-bold text-gray-900">Ücretsiz Kargo 🚚</span></li>
                  </ul>

                  <button onClick={() => handlePaketSec(urun)} disabled={stokYok} className={`w-full py-4 rounded-2xl font-bold text-lg transition-all shadow-md active:scale-95 ${stokYok ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : index === 1 ? 'bg-green-600 text-white hover:bg-green-700 shadow-green-200 hover:shadow-green-300' : 'bg-gray-900 text-white hover:bg-gray-800'}`}>
      ...
      <section id="sss" className="py-24 bg-gray-50 border-t border-gray-100">
        <div className="container mx-auto px-6 max-w-4xl">
            <div className="text-center mb-16">
                <h2 className="text-3xl md:text-4xl font-black text-gray-900">Sıkça Sorulan Sorular</h2>
                <p className="text-gray-500 mt-2">Aklınıza takılan soruların cevapları burada.</p>
            </div>
            <div className="space-y-4">
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <details className="group">
                        ...
                        <p className="text-gray-600 text-sm px-6 pb-6 leading-relaxed">
                            Abonelik kutularımız her ayın 1'i ile 5'i arasında kargoya verilir. Siparişinizi ayın ortasında verdiyseniz, ilk kutunuz hemen ertesi gün kargolanır!
                        </p>
                    </details>
                </div>
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <details className="group">
                        <summary className="flex justify-between items-center font-bold cursor-pointer list-none p-6 text-gray-800">
                            <span> Aboneliğimi istediğim zaman iptal edebilir miyim?</span>
                            <span className="transition group-open:rotate-180">
                                <svg fill="none" height="24" shapeRendering="geometricPrecision" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewBox="0 0 24 24" width="24"><path d="M6 9l6 6 6-6"></path></svg>
                            </span>
                        </summary>
                        <p className="text-gray-600 text-sm px-6 pb-6 leading-relaxed">
                            Elbette! Profil sayfanızdaki "Aboneliklerim" sekmesinden tek tıkla iptal edebilirsiniz. Hiçbir taahhüt veya cayma bedeli yoktur.
                        </p>
                    </details>
                </div>
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <details className="group">
                        ...
                        <p className="text-gray-600 text-sm px-6 pb-6 leading-relaxed">
                            Sürpriz kutu konseptimizde içerikleri uzmanlarımız seçer. Ancak dostunuzun alerjisi veya özel durumu varsa, profil oluştururken belirtebilirsiniz, buna göre özel seçim yaparız.
                        </p>
                    </details>
                </div>
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <details className="group">
                        ...
                        <p className="text-gray-600 text-sm px-6 pb-6 leading-relaxed">
                            Hayır! CanDostumBox ailesinde kargo her zaman ücretsizdir. Sadece kutu ücretini ödersiniz.
                        </p>
                    </details>
                </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            ...
            <div>
              ...
              <ul className="space-y-3 text-sm text-gray-400">
                ...
                <li><a href="#" onClick={(e) => { e.preventDefault(); scrollToSection('sss'); }} className="hover:text-green-400 transition">Sıkça Sorulan Sorular</a></li>
